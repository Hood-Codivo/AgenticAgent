...
def main():
    parser = argparse.ArgumentParser(description="Train the EURUSD PPO trading agent.")
    parser.add_argument(
        "--timesteps",
        type=int,
        default=300000,
    ...
    try:
        ...
        print(f"✗ Error evaluating on train data: {e}")
        equity_curve_train = []
        final_equity_train = 0
    ...
    try:
        ...
        if equity_curve_train:
            plt.plot(equity_curve_train, label="Train (in-sample) equity", linewidth=2)
        if equity_curve_test:
            plt.plot(equity_curve_test, label="Test (out-of-sample) equity", linewidth=2)
